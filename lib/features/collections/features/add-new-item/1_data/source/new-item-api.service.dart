...
abstract class NewItemApiService {
  ...
  Future<Either<String, dynamic>> addBarcodeToGame(AddBarcodeToGameRequest dto);
}
...
class NewItemApiServiceImpl implements NewItemApiService {
  ...
  @override
  Future<Either<String, dynamic>> addBarcodeToGame(AddBarcodeToGameRequest dto) async {
    try {
      final accessToken = await serviceLocator<AuthLocalService>().getAccessToken();
      final response = await serviceLocator<DioClient>().put(
        ApiUrlConstant.addBarcodeToGame,
        data: dto.toJson(),
        options: Options(headers: {
          'Authorization': 'Bearer $accessToken'
        }),
